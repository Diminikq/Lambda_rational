...
$$\lambda{\text{pure}}(x) = \text{sgn}(x) \cdot \sum{k=0}^{\lfloor \log{10}(|x|) \rfloor} \left( \lfloor |x| \cdot 10^{-k} \rfloor \pmod{10} \right) \cdot 10^{\lfloor \log{10}(|x|) \rfloor - k}$$
...
Treating the result this way even perserves involution, i.e. lmabda(lambda(1/3))=lambda(-1/3)=1/3.
